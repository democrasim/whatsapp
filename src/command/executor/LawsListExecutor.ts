...
import { lawToTextSimple } from "@/generator/lawGenerator";
...
export default class LawListExecutor extends CommandExecutor {
    ...
    async execute(command: Command) {
        ...
        if(!['PASSED','VETOED','CANCELED','FAILED',"UNDER_VOTE"].includes(command.props["status"].toUpperCase())) {
            clientModule.client!.sendText(command.group, "That's not a valid argument.");
            return;
        }
        ...
        text += `- Page 1 - Displaying ${laws.length} law${(laws.length > 1) ? 's' : ''}
